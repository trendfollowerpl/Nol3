...
namespace Nol3.Communication.Unit.Tests
{
	...
	public class FIXMLTests
	{
		...
		{
			IdGenerator.Reset();
			string result = FIXML.GenerateRequest<UserRequest>(new UserRequest(),IdGenerator.ID);
			string expected = @"<FIXML v=""5.0"" r=""20080317"" s=""20080314""><UserReq UserReqID="""" UserReqTyp=""1"" Username=""BOS"" Password =""BOS""/></FIXML>";
			var sb = new StringBuilder();
